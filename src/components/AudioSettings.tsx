...
export default function AudioSettings() {
  ...
  return <fieldset className="audio-settings">
    ...
    <label className="setting-row">
      <span><b>Звуки игры</b><small>Удары, мастерская и атмосфера мира</small></span>
      <input type="checkbox" checked={!settings.muted} onChange={event => change({ muted: !event.target.checked })} />
    </label>
    {(['sfx', 'ambience'] as const).map(bus => <label className="setting-row audio-level" key={bus}>
      <span><b>{bus === 'sfx' ? 'Эффекты' : 'Окружение'}</b><small>{Math.round(settings[bus] * 100)}%</small></span>
