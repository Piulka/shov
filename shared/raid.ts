...
export const raidRoles: { id: RaidRole; name: string; description: string }[] = [
  { id: 'rupture', name: 'Урон', description: 'По 2 000 за победу. При трёх победах: за каждую волну до 1 000 × min(урон с 4-й по 8-ю секунду / 1 800, 1); убийство до конца окна даёт 1 000. Ещё до 1 000 × clamp((180 − общее время в секундах) / 90, 0, 1). Дробные бонусы округляются вниз.' },
  { id: 'bulwark', name: 'Защита', description: 'По 2 000 за победу. При трёх победах: по 500 × (1 − урон здоровью от удара / двойную силу врага) за тяжёлые удары на 9-й и 18-й секунде каждой волны; убийство до удара даёт 500. Щиты и защита учитываются, лечение не даёт очков. Ещё до 1 000 × clamp((180 − общее время в секундах) / 90, 0, 1). Бонус каждой волны округляется вниз.' },
  { id: 'cleanse', name: 'Очищение', description: 'По 2 000 за победу. При трёх победах: за каждую волну 1 000 × среднее трёх оценок снятия кровотечения на 6-й, 12-й и 18-й секунде. Оценка = clamp((4 − задержка снятия в секундах) / 3, 0, 1); без снятия 0, убийство в течение 4 секунд после наложения или раньше даёт 1. Ещё до 1 000 × clamp((180 − общее время в секундах) / 90, 0, 1). Бонус каждой волны округляется вниз.' },
];
...
function encounters(role: RaidRole): Enemy[] {
  ...
  return ids.map((id, index) => ({
    ...
    description: role === 'bulwark' ? 'Зачётные тяжёлые удары на 9-й и 18-й секунде.' : role === 'cleanse' ? 'Зачётное кровотечение на 6-й, 12-й и 18-й секунде.' : 'Зачётное окно урона: с 4-й по 8-ю секунду, квота 1 800.',
  }));
}
