...
Действие происходит в королевстве Шовь, где привычные торговые дороги захватывают чудовища. Герой вступает в гильдию искателей и начинает с лесной окраины. За лесом открываются снежные перевалы, а дальше лежат пепельные земли и крепость дракона.

Мир понятен без словаря: воин держит удар мечом и щитом, маг использует огонь, лёд и защиту, лучник поражает врага стрелами и ядом. Класс определяется оружием и набором навыков; игрок свободно сохраняет и меняет сборки одного героя. Нельзя заставлять создавать нового персонажа ради другого оружия.

Начальный набор регионов: Зеленолесье, Морозные горы и Пепельные земли. В каждом два обычных маршрута и один босс. Противники имеют видимую роль: гоблин, слизень, разбойник, волк, тролль, скелет-маг, голем, орк, рыцарь, дракон. Тяжёлые удары предупреждаются, яд и ожоги можно очистить, щиты и лечение помогают пережить опасные серии.

Тон приключенческий и красочный. Мультяшные персонажи, выразительное оружие и читаемые эффекты должны работать на небольшом экране Telegram. Подробности и рабочие пути графики описаны в [актуальном художественном брифе](14-art-audio-handoff.md). Прежняя концепция керамики, стекла и нитей заменена классическим фэнтези.

Название «ШОВЬ» остаётся именем мира и брендом проекта. Латинское написание и технические идентификаторы: `shov`. Игровые ID сохраняются для совместимости сохранений, даже когда их английское слово относится к прежнему сеттингу. Пользовательские имена и названия сборок не переименовываются.
...
| Поручения гильдии | Цели на изучение механик, улучшение сборки и победы над боссами | После первых уровней остаются понятные ориентиры |
...
| Клановые задания | Участники развивают общий клан и выполняют задачи | Асинхронная кооперация, а не гонка присутствия |
...
**Добыча.** Герой повторяет выбранную открытую точку. Игрок выбирает цель из её таблицы: например, лук для сборки на яде или основу конкретного рецепта. Выбор меняет профиль добычи в установленных пределах, но не создаёт дополнительный поток наград.
...
Новые предметы не надеваются автоматически. Автовыбор менял бы билд за спиной игрока. Авторазбор применяется только к явно выбранным категориям и после первых ручных разборов; новые типы легендарных вещей, закреплённые и надетые предметы защищены всегда.
